...
pub struct Genetic {
    pub population: Vec<crate::individual::Individual>,
...
impl Genetic {
    pub fn new(props: GeneticProperties, rng: &mut StdRand) -> Self {
        ...
        Genetic {
            population: (0..pop).map(|_| crate::individual::Individual::new(range_up, range_down, rng)).collect(),
            props,
        }
    }
    
    pub fn sort_population_by_fitness(&mut self, dataset: &Vec<Vec<i32>>) {
        self.population.sort_unstable_by(|ind_a, ind_b| ind_a.fitness(dataset).partial_cmp(&ind_b.fitness(dataset)).unwrap_or(Ordering::Equal));
    }
    ...
    pub fn run(&mut self, generations: usize, dataset: &Vec<Vec<i32>>, rng: &mut StdRand) {
        ...
        for g in 0..generations {
            ...
            for i in 0..reproduction_pop {
                self.population.push(self.population[i].reproduce());
            }
            println!("{}", g);
        }
    }
    ...
    pub fn sort_population_by_complexity(&mut self) {
        self.population.sort_unstable_by(|ind_a, ind_b| ind_a.stack.len().cmp(&ind_b.stack.len()));
    }
}
